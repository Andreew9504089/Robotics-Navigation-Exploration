...
class EnvRunner:
	...
	def run(self, policy_net, value_net):
		...
		for step in range(self.n_step):
			...
			self.mb_actions[step, :], self.mb_a_logps[step, :] = policy_net(torch.from_numpy(self.states).float().to(self.device))
			#self.mb_a_logps[step, :] = ...
			self.mb_values[step, :]  = value_net(torch.from_numpy(self.states).float().to(self.device))
